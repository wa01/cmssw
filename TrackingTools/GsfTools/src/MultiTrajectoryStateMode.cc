...
vector<MultiGaussianState1D>
MultiTrajectoryStateMode::momentumMultiState1DCartesian (const TrajectoryStateOnSurface tsos) const
{
  //
  // clear result vector and check validity of the TSOS
  //
  vector<MultiGaussianState1D> result;
  if ( !tsos.isValid() ) {
    edm::LogInfo("MultiTrajectoryStateMode") << "Cannot calculate mode from invalid TSOS";
    return result;
  }
  ...
  // transformation in 1D multi-states and creation of utility classes
  //
  result.push_back(MultiGaussianState1D(pxStates));
  result.push_back(MultiGaussianState1D(pyStates));
  result.push_back(MultiGaussianState1D(pzStates));
...
vector<MultiGaussianState1D>
MultiTrajectoryStateMode::momentumMultiState1DLocal (const TrajectoryStateOnSurface tsos) const
{
  //
  // clear result vector and check validity of the TSOS
  //
...
vector<MultiGaussianState1D>
MultiTrajectoryStateMode::momentumMultiState1DPPhiEta (const TrajectoryStateOnSurface tsos) const
{
  //
  // clear result vector and check validity of the TSOS
  //
  ...
  // transformation in 1D multi-states and creation of utility classes
  //
  result.push_back(MultiGaussianState1D(pStates));
  result.push_back(MultiGaussianState1D(phiStates));
  result.push_back(MultiGaussianState1D(etaStates));
